__author__ = "David Lähnemann, Johannes Köster, Christian Meesters, Thibault Dayris"
__copyright__ = "Copyright 2023, David Lähnemann, Johannes Köster, Christian Meesters, Thibault Dayris"
...
class Executor(RemoteExecutor):
    ...
    def run_job(self, job: JobExecutorInterface):
        ...
        slurm_logfile = os.path.abspath(f"logs/slurm/{log_folder}/%j.log")
        ...
        if job.resources.get("runtime"):
            ...
            self.logger.warning(
                "No wall time information given. This is not a good practice. "
                "The default value of 6h 0m 0s (360 minutes) was given to your"
                " job submission."
            )
            call += " --time 360"
        ...
        for j in active_jobs:
            ...
            if status == "COMPLETED":
                ...
                msg = (
                    ...
                    f"'{status}'. OUT_OF_MEMORY, or TIMEOUT jobs can be solved "
                    "using either `--attempt X` where X is a natural number "
                    "above 2, or `--retries Y` where Y is a natural number "
                    "above 1."
                )
    ...
    def get_account_arg(self, job: JobExecutorInterface):
        ...
        if job.resources.get("slurm_account"):
            ...
            return f" --account {job.resources.slurm_account}"
        else:
            if self._fallback_account_arg is None:
                ...
                if account:
                    ...
                    self._fallback_account_arg = f" --account {account}"
                else:
                    self.logger.warning(
    ...
    def get_partition_arg(self, job: JobExecutorInterface):
        ...
        if job.resources.get("slurm_partition"):
            ...
            if "gpu" in job.resources.gpu:
                partition = "gpgpuq"
            if job.resources.runtime <= 60 * 6:
                partition = "shortq"
